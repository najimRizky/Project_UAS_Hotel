<footer style="background-color: #334443; color:white;" class="text-center text-lg-start">
  <!-- Grid container -->
  <div class="container p-4">
    <!--Grid row-->
    <div class="row">
      <!--Grid column-->
      <div class="col-lg-4 col-md-12 mb-4 mb-md-0">
        <h5 class="text-uppercase">Isi FOOTER</h5>

        <p style="padding-right: 20px">Website ini dibuat untuk memenuhi nilai Ujian Akhir Semester 4 dari kelar Pemograman Website
                    di kelas IF430-F. Website ini dibuat oleh satu kelompok yang terdiri
                    dari 4 orang yaitu: Najim Rizky, Bonaventura Sanjaya, Filbert Khouwira, Mohamad Farhan</p>
      </div>
      <!--Grid column-->

      <!--Grid column-->
      <div class="col-lg-4 col-md-12 mb-4 mb-md-0">
        <h5 class="text-uppercase">LIBRARY</h5>
            <div class="item">Bootstrap 4</div>
            <div class="item">Jquery</div>
            <div class="item">Codeigniter</div>
            <div class="item">Grocery CRUD</div>
      </div>
      <div class="col-lg-4 col-md-12 mb-4 mb-md-0">
        <h5 class="text-uppercase">ADDRESS</h5>
        <div class="item"><i class="building icon"></i>Universitas Multimedia Nusantara</div>
        <div class="item"><i class="envelope outline icon"></i>burung@gmail.com</div>
        <div class="item"><i class="phone icon"></i>0812-1313-1121</div>
      </div>
      <!--Grid column-->
    </div>
    <!--Grid row-->
  </div>
  <!-- Grid container -->

  <!-- Copyright -->
  <div  class="text-center p-3" style="background-color: #f0f0f0; color:black">
    © 2021 Copyright:
    <a class="text-dark" href="https://mdbootstrap.com/">MDBootstrap.com</a>
  </div>
  <!-- Copyright -->
</footer>